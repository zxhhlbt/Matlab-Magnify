...
function ButtonDownCallback(src,eventdata)
   ...
   set(f1, ...
      ...
      'CurrentAxes',a2);
   set(a2, ...
      'UserData',[2,2,0.2], ...  %magnification, frame size
      'Color',get(a1,'Color'), ...
      'Box','on');
...
function ButtonMotionCallback(src,eventdata)
   ...
   if ~isempty(H)
      ...
      set(a2,'Position',[(f_cp./f_pos(3:4)) 0 0]+a2_param(3)*a1_pos(3)*[-1 -1 2 2]);
      a2_pos = get(a2,'Position');

   	set(a2,'XLim',a1_cp(1)+(1/a2_param(1))*(a2_pos(3)/a1_pos(3))*diff(get(a1,'XLim'))*[-0.5 0.5]);
   	set(a2,'YLim',a1_cp(2)+(1/a2_param(2))*(a2_pos(4)/a1_pos(4))*diff(get(a1,'YLim'))*[-0.5 0.5]);
   end;
return;
...
function KeyPressCallback(src,eventdata)
   ...
   if ~isempty(H)
      ...
      elseif (strcmp(get(f1,'CurrentCharacter'),'-') | strcmp(get(f1,'CurrentCharacter'),'_'))
         a2_param(1) = a2_param(1)/1.2;
      elseif (strcmp(get(f1,'CurrentCharacter'),']'))  % press ']' to increase y magnification
         a2_param(2) = a2_param(2)*1.2;
      elseif (strcmp(get(f1,'CurrentCharacter'),'['))  % press '[' to decrease y magnification
         a2_param(2) = a2_param(2)/1.2;
      elseif (strcmp(get(f1,'CurrentCharacter'),'<') | strcmp(get(f1,'CurrentCharacter'),','))
         a2_param(3) = a2_param(3)/1.2;
      elseif (strcmp(get(f1,'CurrentCharacter'),'>') | strcmp(get(f1,'CurrentCharacter'),'.'))
         a2_param(3) = a2_param(3)*1.2;
      end;
